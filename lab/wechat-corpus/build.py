...
env = {**os.environ, 'CARGO_TARGET_DIR': str(target),
       'MAKEPAD_HTML_PATCHSET': 'wechat-css-1+table-2+text-shadow-3+inline-4' if args.variant == 'patched' else 'none'}
...
with tempfile.TemporaryDirectory(prefix='makepad-html-upstream-') as temporary:
    ...
    if args.variant != 'patched':
        ...
        text = text.replace('members = [".", "vendor/blitz/packages/*", "vendor/parley"]', 'members = ["."]')
