...
NOTES={
 ...
 '17-inline-baselines':'原版缺少上下标偏移和 ruby 注音布局，行内盒基线及标签之间空白也有差异。',
...
PATCH_NOTES={
 '01-typography':'已补上下标、嵌套基线偏移及跨标签空格；中文两端对齐、装饰和字体仍存在差异。',
 '07-links-footnotes':'已补 sup 与行内盒 middle 基线，保留链接点击事件；行内边框、字体与自动列宽仍需完善。',
 '11-ruby':'水平 HTML ruby/rt 已在正文上方原生排版，sup/sub 已应用基线偏移。复杂 ruby、竖排和跨行分段尚未实现。',
 '17-inline-baselines':'长度/百分比、嵌套上下标、跨标签空格和行内盒 top/bottom/middle/末行基线已修复；逐段几何差仍需查看测量，不代表完整 CSS inline/ruby 兼容。',
 '04-tables':'已修复合并单元格内横线、单元格居中与混合边框；自动列宽和虚线节奏仍与 WebKit 不完全一致。原生水平滚动已接入持久文档会话。',
 ...
 '16-text-shadows':'已补偏移、模糊、多重阴影、currentColor 继承、none 覆盖、透明文字和文字装饰阴影。已修复跨标签空格；字体和下划线位置仍与 WebKit 不同。',
 '08-carousel':'已修复仅含卡片的 nowrap 图集纵排；Makepad 内部横向滚动和滚动后链接命中已通过原生测试。混合文本与卡片的完整换行规则仍待补齐。',
...
report={'method':'Raw-engine diagnostic, exact identical fixture bytes and resources. Real macOS WKWebView; page JS disabled. 2x. No registration or alignment. CSS font-family PingFang SC in both.',
 ...
 'variants':{'patched':'Same extended features + repository vendor/blitz patches wechat-css-1 + table-2 + text-shadow-3 + inline-4','media':'woff + gif; other product defaults','extended':'woff + gif + blitz-paint/svg + blitz-dom/floats; WebP decoder enabled transitively by SVG'},
...
<div class="card">17 个样本 × 390 / 600 CSS px。来源：doocs/md、Markdown Nice；少量边界探针另有标注。<br>原始 HTML 和资源一致，字体声明均为 PingFang SC，页面 JS 关闭。右侧是真实 macOS WKWebView；这不是 iOS 或微信客户端验收。<br><a href="README.md">研究结论与修改路线</a> · <a href="corpus.json">样本及来源</a> · <a href="results.json">原始测量</a></div>
