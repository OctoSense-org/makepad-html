...
已实现 rowspan/colspan 边界裁剪、rowspan=0 与行组边界、逐边冲突解析、单元格半边框尺寸、dashed/dotted/double 绘制，以及块级/行内内容的 top/middle/bottom 对齐。冲突来源包括单元格、行、行组、列、列组与 table；支持 RTL，样式更新后重新计算。

默认 27 项、扩展 29 项测试通过；其中新增 9 项表格回归，前 8 项在旧版本全部失败。原生 Makepad 截图/滚动验证通过。15 个样本 × 两种宽度共 30 组完成；原有 13 个非表格样本的 26 张图与上一版逐字节一致。原表格样本最大探针差 6.75 → 0.75px；新增边界样本 66 → 2px。自动列宽、跨度边界高度、虚线节奏和角点仍有差异。

[完整测量及剩余工作](../../docs/table-parity.md) · [新表格探针](fixtures/15-table-edges.html)。生产资源策略未变，仍未接入 Makepad 内部横向滚动。
...
前 14 个文件是依据上述模式编写的回归探针，中文测试文案和图片校准图由本实验编写，不是从真实公众号完整导出的文章。12 号的部分 CSS 效果和 13 号定位/浮动是明确标注的探索性边界探针，未验证微信发布接受度。14 号测试宿主导入行为。15 号是依据 CSS 表格规则编写的引擎回归样本。它们不是“微信官方 HTML/CSS 全集”。
